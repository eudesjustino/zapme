...
window.WAPI.sendMessageText = function (idMessage, to, message, done) {

    WAPI.sendMessage(to, message, function (OK) {
        if (OK) {
            done({
                status: 'OK',
                message: {
...
window.WAPI.sendMessageMedia = function(idMessage,imgBase64, to, filename, caption,done){
    ...
    WAPI.sendImage(imgBase64, to, filename, caption,(OK)=>{
        if (OK) {
            done({
                status: 'OK',
                message: {
...
window.WAPI.downloadFile2 = function (url, success,error) {
    let xhr = new XMLHttpRequest();
    xhr.onload = function () {
        if (xhr.readyState == 4) {
            if (xhr.status == 200) {
...
WAPI.processMediaMessage = (media,key,type,mimetype) => {
    ...
    if (media) {
        ...
        return new Promise((resolver,reject) => {
            ...
            WAPI.downloadFile2(media, (arrBuffer) => {
                console.log(arrBuffer);
                console.log('key ',key);
                console.log('mimetype ',mimetype);
                console.log('type ',type);
                
                Store.CryptoLib.decryptE2EMedia(type.toUpperCase(), arrBuffer, key, mimetype).then(
...
async function transformData(message) {
    ...
    messageObject.isForwarded = message.isForwarded;
    messageObject.forwardingScore = message.forwardingScore

    messageObject.from = {};
    messageObject.from.number = message.from.user
    messageObject.from.name = message.sender.pushname;
    messageObject.from.isMyContact = message.sender.isMyContact;
    messageObject.from.profileImageThumObj = message.sender.img;

    messageObject.message = {};

    if (message.type == "chat" && message.subtype == "url") {
        messageObject.message.type = 'chat-url';
        messageObject.message.description = message.description;
        messageObject.message.title = message.title;
        messageObject.message.content = message.content;
        messageObject.message.thumbnail = message.thumbnail;
    }

    if (message.type == "chat") {
        messageObject.message.type = message.type;
        messageObject.message.mimetype = message.mimetype;
        messageObject.message.content = message.content;
    }
    ...
    if (message.type == "image") {
        messageObject.message.type = message.type;
        messageObject.message.mimetype = message.mimetype;
        const dados = await WAPI.processMediaMessage(message.clientUrl, message.mediaKey, message.type, message.mimetype);
        messageObject.message.content = dados;
        messageObject.message.description = message.description;
        messageObject.message.title = message.title;
        messageObject.message.size = message.size;
        messageObject.message.width = message.width;
        messageObject.message.height = message.height;
        messageObject.message.isGif = message.isGif;
    }
    ...
    if (message.type == "sticker") {
        ...
        messageObject.message.mimetype = message.mimetype;
        messageObject.message.content = message.content;
        messageObject.message.caption = message.caption;
        messageObject.message.size = message.size;
    }
    ...
    if (message.type == "video") {
        ...
        messageObject.message.height = message.height;
        messageObject.message.isGif = message.isGif;
        messageObject.message.duration = message.duration;
    }

    if (message.type == "ptt") {
        messageObject.message.type = message.type;
        if (message.mimetype == "audio/ogg; codecs=opus") {
            messageObject.message.mimetype = "audio/ogg; codecs=opus".split(';')[0]
        } else {
            messageObject.message.mimetype = message.mimetype;
        }
        const dados = await WAPI.processMediaMessage(message.clientUrl, message.mediaKey, message.type, message.mimetype);
    ...
    if (message.type == "document") {
        ...
        messageObject.message.description = message.description;
        messageObject.message.size = message.size;
        messageObject.message.pageCount = message.pageCount;
        messageObject.message.height = message.height;
        messageObject.message.filename = message.filename;
        messageObject.message.caption = message.caption;
        messageObject.message.title = message.title;
    }
    ...
    if (message.type == "vcard") {
        messageObject.message.type = message.type;
        messageObject.message.contactName = message.subtype;
        messageObject.message.content = message.content;
    }

    if (message.type == "location") {
        messageObject.message.type = message.type;
        messageObject.message.comment = message.comment;
        messageObject.message.lat = message.lat;
        messageObject.message.lng = message.lng;
        messageObject.message.sequence = message.sequence;
        messageObject.message.shareDuration = message.shareDuration;
    }
...
WAPI.waitNewMessages(false, async (data) => {
    ...
    data.forEach(async (message) => {
       // if (message.isGroupMsg == false) {
            const data = await transformData(message);
...
const targetImgQrCode = document.querySelector('body');

const targetConnected = document.querySelector("div[id='app']");

function handleMutationObserverImgQrCode(mutations) {
    let event = "";
    mutations.forEach(function (mutation) {
        if (mutation.target.className === '_1pw2F') {
            const eventTargt = mutation.target.dataset['ref'];
            if (event !== eventTargt) {
                event = eventTargt;
                window.generatedQRcode(mutation.target.dataset['ref']);
            }
        }
    });
}

function handleMutationObserverReloadQRCode(mutations) {
    mutations.forEach(function (mutation) {
        if (mutation.target.className === '_1MOym') {
            document.querySelector("._1MOym").click();
        }

    });
}

function handleMutationObserverDectectOpenMutiplePages(mutations) {
    mutations.forEach(function (mutation) {        
        if (mutation.target.className === '_1FPJ- app-wrapper-web os-mac') {
            let message = document.querySelector("._2Vo52").innerText;
            message =message.split('.')[0]
            window.openPageConflict(message);
            document.querySelector("._2eK7W._3PQ7V").click();
        }
...
function handleMutationObserverConnected(mutations) {
    mutations.forEach(function (mutation) {
        if (mutation.addedNodes.length > 0 && mutation.addedNodes[0].className == 'app _3fUe9 two') {
            window.connected();            
        }

        if (mutation.removedNodes.length > 0 && mutation.removedNodes[0].className == 'app _3fUe9 two') {
            window.disconnected();
        }
...
let observerReloadQRCode = new MutationObserver(handleMutationObserverReloadQRCode);
let observerImgQrCode = new MutationObserver(handleMutationObserverImgQrCode);
let observerConnected = new MutationObserver(handleMutationObserverConnected);
let observerDectectOpenMutiplePages = new MutationObserver(handleMutationObserverDectectOpenMutiplePages);


observerReloadQRCode.observe(targetConnected, { childList:true, subtree: true, attributeOldValue: true });
observerImgQrCode.observe(targetImgQrCode, { childList:true, subtree: true, attributeOldValue: true });
observerConnected.observe(targetConnected, {childList:true, subtree: true, attributeOldValue: true});
observerDectectOpenMutiplePages.observe(targetConnected, {childList:true, subtree: true, attributeOldValue: true});
